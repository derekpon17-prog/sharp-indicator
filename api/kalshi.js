...
module.exports = async function handler(req, res) {
  ...
  // TEMP DIAGNOSTIC 2026-08-27 (per Derek): pull a real day of Novig's free public data
  // to actually show him real data, not just documentation. Added here instead of a new
  // file -- the project is exactly at the Hobby plan's 12-function ceiling, confirmed live
  // (a standalone file broke the deployment: exceeded_serverless_functions_per_deployment).
  // Meant to be removed after review, not a committed feature.
  if (req.query && req.query.novigSample) {
    try {
      ...
      if (latestMarketDate) {
        ...
        out.marketsFile = {
          date: latestMarketDate, totalMarketsListed: mRows.length, sportsMarketsListed: sportsRows.length,
          byLeague, sampleRows: sportsRows.slice(0, 5),
          mostVolumeToday: sportsRows.filter(r => parseFloat(r.dailyVolume) > 0)
            .sort((a, b) => parseFloat(b.dailyVolume) - parseFloat(a.dailyVolume)).slice(0, 5),
        };
      }
      ...
      if (latestTradeDate) {
        ...
        out.tradesFile = {
          date: latestTradeDate, totalRowsInFile: tLines.length - 1, sportsTakerTrades: takerRows.length,
          byLeagueTradeCounts: byLeagueTrades, sampleRows: takerRows.slice(0, 5),
        };
      }
