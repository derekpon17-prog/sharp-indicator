...
function calcPin(fairProb,softImps){
  if(!softImps.length) return 0;
  const avg = softImps.reduce((a,b)=>a+b,0)/softImps.length;
...
function calcMoney(novOk,open,current,softImps){
  let s=0;
  if(novOk) s+=42;
  if(open!==null&&open!==undefined){ const v=Math.abs(current-open); s+=v>=20?32:v>=12?26:v>=7?18:v>=3?10:v>=1?4:0; }
  if(softImps.length>1){ const sp=Math.max(...softImps)-Math.min(...softImps); s+=sp>=0.08?26:sp>=0.05?20:sp>=0.03?12:sp>=0.015?6:0; }
...
function analyze(game){
  ...
  for(const mkey of ['h2h','spreads','totals']){
    ...
    if(!pm||pm.outcomes.length<2||!sms.length) continue;
    ...
    for(let i=0;i<pm.outcomes.length;i++){
      ...
      if(si>bestSI&&ps>0){
        ...
        best={
          id:game.id, away:game.away_team, home:game.home_team,
          commenceTime:game.commence_time, sharpSide:side, market:mkey,
          siScore:si, pillars:{rlm,pinnacle:Math.round(ps),money:Math.round(ms)},
...
module.exports = async function handler(req, res) {
  ...
  try{
    ...
    const plays = upcoming.map(analyze).filter(p=>p!==null&&p.pillars.pinnacle>0).sort((a,b)=>b.siScore-a.siScore);
