...
/* NHL/NBA FIX 2026-09-16 (per Derek, applied proactively rather than waiting for the same
   bug WNBA just had). WNBA's real incident -- real games with scores up to 99 sitting
   20-27 hours out, invisible to a 3h window -- came from playing evening-arena slates
   rather than spreading games through the day like MLB. NHL and NBA are the same kind of
   sport: evening tip-off/puck-drop games, not a daily grind. NHL starts this Saturday;
   waiting to confirm the identical failure live again, rather than applying the lesson
   already learned, would just repeat a known mistake. Widened both to 24h alongside WNBA.
   Genuinely unconfirmed against real season data for either yet -- flagged here so a
   future check knows this was reasoned from WNBA's pattern, not independently verified,
   and should be revisited with real games once each season is underway. */
const NOVIG_WINDOW_BY_SPORT = { NCAAF:96, NFL:48, MLB:3, NBA:24, NHL:24, WNBA:24 };
...
module.exports=async function handler(req,res){
  ...
  if(req.query&&req.query.nbaMktCheck){
    try{
      const r=await fetch('https://gql.novig.us/v1/graphql',{
